...
export function appendQqMessage(msg: QqMessage) {
  try {
...
export function queryQqMessages(opts: { contact?: string; days?: number; limit?: number }): QqMessage[] {
  const { contact, days = 7, limit = 50 } = opts
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString()
  try {
    const lines = fs.readFileSync(MESSAGES_FILE, 'utf8').split('\n').filter(Boolean)
    return lines
      .map(l => { try { return decryptLine(l) as QqMessage } catch { return null } })
      .filter((m): m is QqMessage => m !== null && m.timestamp >= cutoff)
      .filter(m => !contact || m.from.toLowerCase().includes(contact.toLowerCase()) || String(m.uin).includes(contact))
      .slice(-limit)
      .reverse()
  } catch { return [] }
}
