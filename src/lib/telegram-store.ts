...
export function appendTelegramMessage(msg: TelegramMessage) {
  try {
...
export function queryTelegramMessages(opts: { contact?: string; days?: number; limit?: number }): TelegramMessage[] {
  const { contact, days = 7, limit = 50 } = opts
  const cutoff = new Date(Date.now() - days * 86_400_000).toISOString()
  try {
    const lines = fs.readFileSync(MESSAGES_FILE, 'utf8').split('\n').filter(Boolean)
    return lines
      .map(l => { try { return decryptLine(l) as TelegramMessage } catch { return null } })
      .filter((m): m is TelegramMessage => m !== null && m.timestamp >= cutoff)
